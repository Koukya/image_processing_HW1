...
def lbp(kernal_size,image):
    ...
    value = image
    for y in range(1,rows-kernal_size+1):
        row = y 
        for x in range(1,cols-kernal_size+1):
            col = x 
            ...
            +2**3*kernal_operation(image[row,col+1],image[row,col])
            +2**2*kernal_operation(image[row+1,col-1],image[row,col])
            +2**1*kernal_operation(image[row+1,col],image[row,col])
            +2**0*kernal_operation(image[row+1,col+1],image[row,col])
...
#cv2.createTrackbar('max','try',100,1000,lambda x: x)



cv2.imshow('1',image_original)
cv2.imshow('2',image_gray)
cv2.imshow('3',image_guass)
cv2.imshow('4',image_canny)
